...
@pytest.mark.parametrize(
    "model_name",
    list_models(exclude_filters=EXCLUDE_FILTERS + NON_STD_FILTERS, include_tags=True),
)
@pytest.mark.parametrize("batch_size", [1])
def test_model_default_cfgs(model_name, batch_size):
    ...
    if all([x <= MAX_FWD_OUT_SIZE for x in input_size]) and not any(
        ...
        assert (
            outputs.shape[spatial_axis[0]] == pool_size[0]
        ), f"unpooled feature shape {outputs.shape} != config"
        assert (
            outputs.shape[spatial_axis[1]] == pool_size[1]
        ), f"unpooled feature shape {outputs.shape} != config"
        assert (
            outputs.shape[feat_axis] == model.num_features
        ), f"unpooled feature dim {outputs.shape[feat_axis]} != model.num_features {model.num_features}"
        assert (
            outputs_pre.shape[1] == model.head_hidden_size
        ), f"pre_logits feature dim {outputs_pre.shape[1]} != model.head_hidden_size {model.head_hidden_size}"
        ...
        assert (
            outputs.shape[1] == model.head_hidden_size
        ), f"feature dim w/ removed classifier {outputs.shape[1]} != model.head_hidden_size {model.head_hidden_size}"
        assert (
            outputs.shape == outputs_pre.shape
        ), f"output shape of pre_logits {outputs_pre.shape} does not match reset_head(0) {outputs.shape}"
        ...
        if not isinstance(model, EARLY_POOL_MODELS):
            ...
            assert (
                outputs.shape[spatial_axis[0]] == pool_size[0]
                and outputs.shape[spatial_axis[1]] == pool_size[1]
            )
...
@pytest.mark.parametrize(
    "model_name",
    list_models(
        filter=NON_STD_FILTERS,
        exclude_filters=NON_STD_EXCLUDE_FILTERS,
        include_tags=True,
    ),
)
@pytest.mark.parametrize("batch_size", [1])
def test_model_default_cfgs_non_std(model_name, batch_size):
    ...
    if feat_dim is None:
        feat_dim = -1 if outputs.ndim == 3 else 1
    assert (
        outputs.shape[feat_dim] == model.head_hidden_size
...
def test_model_forward_features(model_name, batch_size):
    ...
    assert (
        len(expected_channels) >= 3
    )  # all models here should have at least 3 default feat levels
...
@pytest.mark.parametrize(
    "model_name",
    list_models(
        module=FEAT_INTER_FILTERS, exclude_filters=EXCLUDE_FILTERS + ["*pruned*"]
    ),
)
@pytest.mark.parametrize("batch_size", [1])
def test_model_forward_intermediates_features(model_name, batch_size):
    """Run a single forward pass with each model in feature extraction mode"""
    model = create_model(
        model_name, pretrained=False, features_only=True, feature_cls="getter"
    )
    model.eval()
    expected_channels = model.feature_info.channels()
    expected_reduction = model.feature_info.reduction()
...
def test_model_forward_intermediates(model_name, batch_size):
    ...
    assert (
        len(expected_channels) >= 3
    )  # all models here should have at least 3 feature levels
...
if "GITHUB_ACTIONS" not in os.environ:
    ...
    def test_model_forward_torchscript_with_features_fx(model_name, batch_size):
        ...
        ), f"{model_name=} not supported for this test"

        input_size = _get_input_size(model_name=model_name, target=TARGET_JIT_SIZE)
        assert (
            max(input_size) <= MAX_JIT_SIZE
        ), "Fixed input size model > limit. Pick a different model to run this test"
