    ...
    def __init__(
        ...
        if method == "fx":
            ...
            if use_regex:
                ...
                matched = [
                    g for g in node_names if any([fnmatch.fnmatch(g, n) for n in names])
                ]
            if not matched:
            ...
            if use_regex:
                regexes = [re.compile(r) for r in names]
                matched = [
                    m for m in module_names if any([r.match(m) for r in regexes])
                ]
            else:
                matched = [
                    m
                    for m in module_names
                    if any([fnmatch.fnmatch(m, n) for n in names])
                ]
            if not matched:
            ...
            self.hooks = FeatureHooks(
                matched, model.named_modules(), default_hook_type=hook_type
            )
