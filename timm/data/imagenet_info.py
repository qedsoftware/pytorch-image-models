...
class ImageNetInfo(DatasetInfo):
    ...
    def __init__(self, subset: str = "imagenet-1k"):
        ...
        definition_data = pkgutil.get_data(
            __name__, os.path.join("_info", _DEFINITION_FILE)
        )
        reader = csv.reader(
            definition_data.decode("utf-8").splitlines(), delimiter="\t"
        )
    ...
    def label_descriptions(
        ...
        if as_dict:
            return {
                label: self.label_name_to_description(label, detailed=detailed)
                for label in self._synsets
            }
        else:
            return [
                self.label_name_to_description(label, detailed=detailed)
                for label in self._synsets
            ]

    def index_to_label_name(self, index) -> str:
        assert (
            0 <= index < len(self._synsets)
