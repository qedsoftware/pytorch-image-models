    ...
    if "tar" in split or ".." in split:
        ...
        if "::" not in split:
            ...
            if (
                split_idx
                and "splits" in info
                and split_parts[split_idx] in info["splits"]
            ):
        ...
        if split_name:
            ...
            if not num_samples:
                _fc = {
                    f: c
                    for f, c in zip(
                        split_info["filenames"], split_info["shard_lengths"]
                    )
                }
...
class ReaderWds(Reader):
    ...
    def _lazy_init(self):
        ...
        # init data pipeline
        abs_shard_filenames = [
            os.path.join(self.root, f) for f in self.split_info.filenames
        ]
            ...
            pipeline.extend(
                [
                    ...
                    wds.shuffle(
                        ...
                        rng=random.Random(
                            self.worker_seed
                        ),  # this is why we lazy-init whole DataPipeline
                    ),
                ]
            )
        else:
    ...
    def _num_samples_per_worker(self):
        num_worker_samples = self.num_samples / max(
            self.global_num_workers, self.dist_num_replicas
        )
