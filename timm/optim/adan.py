...
class Adan(Optimizer):
    ...
    def __init__(
        ...
        defaults = dict(
            lr=lr, betas=betas, eps=eps, weight_decay=weight_decay, no_prox=no_prox
        )
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                exp_avg.lerp_(grad, 1.0 - beta1)  # m_t
                exp_avg_diff.lerp_(grad_diff, 1.0 - beta2)  # diff_t (v)
                update = grad + beta2 * grad_diff
                exp_avg_sq.mul_(beta3).addcmul_(
                    update, update, value=1.0 - beta3
                )  # n_t
